  ...
  list: [
    {
      ...
      iconPath: "static/image/day/首页.png",
      selectedIconPath: "static/image/day/首页.png",
      text: "首页"
    },
    {
    ...
    },
    {
      ...
      iconPath: "static/image/day/我的预订.png",
      selectedIconPath: "static/image/day/我的预订.png",
      text: "我的预订"
    },
    {
      pagePath: "pages/tabBar/profile/profile",
      iconPath: "static/image/day/个人中心.png",
      selectedIconPath: "static/image/day/个人中心.png",
      text: "个人中心"
    }
  ]
};
...
let oe, ae;
function ce() {
  return oe || (oe = wx$1.canIUse("getAppBaseInfo") && wx$1.canIUse("getDeviceInfo") ? { ...index$1.getAppBaseInfo(), ...index$1.getDeviceInfo() } : index$1.getSystemInfoSync()), oe;
}
function ue() {
  let e2, t2;
  try {
  ...
  return function(t2, n2 = {}) {
    ...
    return new Proxy({}, { get(s3, c2) {
      ...
      return function({ fn: e3, interceptorName: t3, getCallbackArgs: n3 } = {}) {
        ...
        if (a2 && index$1.hideLoading(), y2 && y2.token && y2.tokenExpired && (ie(y2), Y(H.REFRESH_TOKEN, { ...y2 })), g2) {
          let e3 = m2;
          if (p2 && o2) {
...
const dbSet = (name, value) => {
  ...
  data[name] = value;
  index$1.setStorageSync("$$STAT__DBDATA:" + appid, data);
};
const dbGet = (name) => {
  let data = index$1.getStorageSync("$$STAT__DBDATA:" + appid) || {};
  if (!data[name]) {
    let dbdata = index$1.getStorageSync("$$STAT__DBDATA:" + appid);
    if (!dbdata) {
      dbdata = {};
    }
    if (!dbdata[name]) {
...
const dbRemove = (name) => {
  ...
  if (data[name]) {
    ...
    data = index$1.getStorageSync("$$STAT__DBDATA:" + appid);
    if (data[name]) {
      delete data[name];
      index$1.setStorageSync("$$STAT__DBDATA:" + appid, data);
    }
...
function getUuid() {
  ...
  try {
    uuid = index$1.getStorageSync(UUID_KEY);
  } catch (e2) {
    uuid = UUID_VALUE;
  }
  if (!uuid) {
    ...
    try {
      index$1.setStorageSync(UUID_KEY, uuid);
    } catch (e2) {
      index$1.setStorageSync(UUID_KEY, UUID_VALUE);
    }
  }
  return uuid;
}
...
const get_pack_name = () => {
  ...
  if (get_platform_name() === "wx" || get_platform_name() === "qq") {
    if (index$1.canIUse("getAccountInfoSync")) {
      packName = index$1.getAccountInfoSync().miniProgram.appId || "";
    }
  }
  if (get_platform_name() === "n")
    ;
...
const get_scene = (options) => {
  ...
  if (platformName === "wx") {
    scene = index$1.getLaunchOptionsSync().scene;
  }
  return scene;
};
...
const get_page_route = (pageVm) => {
  ...
  let lastPageRoute = index$1.getStorageSync("_STAT_LAST_PAGE_ROUTE");
  if (!page)
...
const is_report_data = () => {
  return new Promise((resolve2, reject) => {
    ...
    let report_status = 1;
    try {
      start_time = index$1.getStorageSync(Report_Data_Time);
      report_status = index$1.getStorageSync(Report_Status);
    } catch (e2) {
      start_time = "";
      report_status = 1;
    }
...
const requestData = (done) => {
  ...
  index$1.request({
    ...
    fail: (e2) => {
      ...
      try {
        report_status_code = index$1.getStorageSync(Report_Status);
      } catch (e22) {
...
class Report {
  constructor() {
    ...
    this._query = {};
    let registerInterceptor = typeof index$1.addInterceptor === "function";
  ...
  addInterceptorInit() {
    let self = this;
    index$1.addInterceptor("setNavigationBarTitle", {
      invoke(args) {
  ...
  interceptRequestPayment() {
    let self = this;
    index$1.addInterceptor("requestPayment", {
      success() {
  ...
  applicationHide(self, type) {
    ...
    index$1.setStorageSync("_STAT_LAST_PAGE_ROUTE", route);
  ...
  sendRequest(optionsData) {
    {
      if (!index$1.__stat_uniCloud_space) {
        console.error(
          "应用未关联服务空间，统计上报失败，请在uniCloud目录右键关联服务空间."
        );
        return;
      }
      const uniCloudObj = index$1.__stat_uniCloud_space.importObject(
        "uni-stat-receiver",
        {
          customUI: true
        }
...
class Stat extends Report {
  static getInstance() {
    if (!index$1.__stat_instance) {
      index$1.__stat_instance = new Stat();
    }
    {
      ...
      if (!index$1.__stat_uniCloud_space) {
        if (space && Object.keys(space).length !== 0) {
          ...
          index$1.__stat_uniCloud_space = tr.init(spaceData);
        } else {
          console.error("应用未关联服务空间，请在uniCloud目录右键关联服务空间");
        }
      }
    }
    return index$1.__stat_instance;
  }
  constructor() {
    super();
  }
  ...
  pushEvent(options) {
    ...
    if (index$1.getPushClientId && ClientID) {
      index$1.getPushClientId({
        success: (res) => {
...
function main() {
  {
    {
      index$1.report = function(type, options) {
      };
    }
  }
}
